...
def test_lines_are_ordered_top_to_bottom():
    lines = group_lines([box("dòng hai", 10, 60), box("dòng một", 10, 10)])
    assert [l.text for l in lines] == ["dòng một", "dòng hai"]
...
def test_a_big_heading_does_not_swallow_the_lines_under_it():
    ...
    heading = box("CẢM ỨNG ĐIỆN TỪ", 10, 0, w=300, h=80)
    body = [box(f"ý {i}", 10, 100 + i * 30) for i in range(4)]
    lines = group_lines([heading, *body])
    assert len(lines) == 5


def test_a_line_is_as_confident_as_its_weakest_box():
    lines = group_lines([box("chắc", 10, 10, score=0.99), box("mờ", 60, 10, score=0.41)])
...
def test_page_summary_flags_but_keeps_low_confidence_lines():
    lines = group_lines([box("rõ", 10, 10, score=0.97), box("nhoè", 10, 60, score=LOW_CONFIDENCE - 0.1)])
    summary = page_summary(lines)
    assert summary["text"] == "rõ\nnhoè", "low-confidence text is flagged, never dropped"

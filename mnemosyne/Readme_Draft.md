...
- [ ] **TODO before publishing this README:** a `quiz.rs` handler + migration 0004 (`quiz_questions` table) clearly implement a Quiz feature (same `describe_llm_failure`/token-logging pattern as the other AI endpoints), but its exact route (`#[post(...)]`/`#[get(...)]` path) was never confirmed before the previous session lost device access. Read `backend/src/handlers/quiz.rs` (the decorator just above `pub async fn`) and `backend/src/main.rs` (the matching `.service(...)`), then add a row for it to the API Overview table below and delete this TODO line.
...
| *(quiz — see TODO above)* | *not yet documented, route unconfirmed* |
